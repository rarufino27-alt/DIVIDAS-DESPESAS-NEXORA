...
function defaultDB(){return{balance:0,transactions:[],debts:[],cards:[],loans:[],recurring:[],cashbooks:[],categories:{receita:['Salário','Trabalho','Freelance','Investimentos','Outros'],despesa:['Moradia','Alimentação','Transporte','Saúde','Educação','Lazer','Assinaturas','Cartões','Empréstimos','Impostos','Outros']},settings:{theme:'light',project:'RAFLO FINANCE',weekMode:'seg-sex'}}}
...
function showAuth(mode='login',message=''){
  ...
  document.querySelector('#auth-form').onsubmit=async e=>{
    ...
    }catch(err){console.error('[RAFLO][AUTH]',err);showAuth(isReg?'register':'login',authErrorMessage(err));}
...
function showUserMenu(user){
  ...
  const name=esc(user?.user_metadata?.full_name||'Usuário RAFLO FINANCE');
...
async function connectUserWorkspace(){
  ...
  }catch(err){console.error('[RAFLO][SUPABASE]',err);setSyncStatus('V1.15 • Supabase indisponível');toast(authErrorMessage(err))}
}
async function initSupabase(){
  if(!window.supabase||!window.supabase.createClient)throw new Error('Biblioteca Supabase não carregada');
  supabaseClient=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true},global:{headers:{'x-nexora-client':'raflo-finance-v1.14'}}});
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&syncReady&&!syncDirty)pullRemoteState().catch(err=>console.error('[RAFLO][PULL]',err))});
window.addEventListener('focus',()=>{if(syncReady&&!syncDirty)pullRemoteState().catch(err=>console.error('[RAFLO][PULL]',err))});
...
function createRecurringRule({type,value,date,category,person,paymentSource,description}){
  const id=uid();
  const rule={id,description:description||category||'Pagamento recorrente',type,value:num(value),day:Math.max(1,Math.min(31,new Date(iso(date)).getDate())),category:category||'',person:person||'',paymentSource:paymentSource||'',active:true,lastGenerated:null};
  db.recurring=db.recurring||[]; db.recurring.push(rule);
  return id;
}
function transactionForm(type,id=null){const x=id?db.transactions.find(z=>z.id===id):null;const title=x?'Editar lançamento':type==='receita'?'Nova Receita':'Nova Despesa';const sourceLabel=type==='receita'?'Origem da receita / de onde veio':'Fonte do pagamento / de onde sairá o dinheiro';layout(title,`<div class="card"><form id="entry-form" class="form"><div class="field"><label>Data de vencimento</label><input id="date" type="date" value="${x?.date||today()}" required></div><div class="field"><label>Valor (R$)</label><input id="value" type="number" step="0.01" min="0" value="${x?.value??''}" required></div><div class="field"><label>Categoria</label><select id="category"><option value="">Selecione</option>${categories(type)}</select></div><div class="field"><label>${type==='receita'?'Quem pagou / origem':'Credor / beneficiário'}</label><input id="person" value="${esc(x?.person||'')}" placeholder="Pessoa, empresa, instituição..."></div><div class="field"><label>${sourceLabel}</label><input id="paymentSource" value="${esc(x?.paymentSource||'')}" placeholder="Conta, carteira, caixa, banco, cartão..."></div><div class="field"><label>Tipo de pagamento</label><select id="paymentType"><option value="unico">Único</option><option value="parcelado">Parcelado</option><option value="recorrente">Recorrente — mesmo valor e mesma data</option></select></div><div class="field"><label>Status</label><select id="status"><option value="previsto">Previsto</option><option value="pendente">Pendente</option><option value="atrasado">Atrasado</option><option value="pago">Pago / Recebido</option></select></div><div class="field"><label>${type==='receita'?'Precisa devolver?':'Semana financeira'}</label>${type==='receita'?`<select id="repayable"><option value="nao">Não</option><option value="sim">Sim</option></select>`:`<select id="week"><option value="auto">Automática — ${weekLabel(x?.date||today())}</option>${[1,2,3,4,5].map(n=>`<option value="${n}">Semana ${n}</option>`).join('')}</select>`}</div>${type==='receita'?`<div class="field"><label>Data limite para devolver (opcional)</label><input id="repayDate" type="date" value="${x?.repayDate||''}"></div>`:''}<div class="field full"><label>Observação</label><textarea id="note" placeholder="Detalhes, referência, parcela ou comentário...">${esc(x?.note||'')}</textarea></div><div class="actions full"><button type="submit" class="btn primary">${x?'Salvar alterações':'Salvar lançamento'}</button><button type="button" class="btn secondary" data-view="movimentos">Cancelar</button></div></form></div>`);if(x){$('#category').value=x.category||'';$('#status').value=x.status||'previsto';if($('#paymentType'))$('#paymentType').value=x.recurringId?'recorrente':(x.installment?'parcelado':'unico');if(type==='receita'){$('#repayable').value=x.repayable?'sim':'nao'}else $('#week').value=x.weekAssigned||'auto'}$('#entry-form').onsubmit=e=>{e.preventDefault();const paymentType=$('#paymentType')?.value||'unico';const data={date:$('#date').value,value:num($('#value').value),category:$('#category').value,person:$('#person').value,paymentSource:$('#paymentSource').value,status:$('#status').value,weekAssigned:type==='receita'?weekOf($('#date').value):($('#week').value==='auto'?weekOf($('#date').value):Number($('#week').value)),note:$('#note').value,type,paymentType};if(paymentType==='recorrente'&&!editingId){data.recurringId=createRecurringRule({type,value:data.value,date:data.date,category:data.category,person:data.person,paymentSource:data.paymentSource,description:data.note||data.category});}if(type==='receita'){data.repayable=$('#repayable').value==='sim';data.repayDate=$('#repayDate').value||''}if(editingId){Object.assign(db.transactions.find(z=>z.id===editingId),data);editingId=null;toast('Lançamento atualizado')}else{db.transactions.push({id:uid(),...data});toast('Lançamento salvo')}save();render(type==='receita'?'receitas':'movimentos')}
}
...
function config(){
  layout('Configurações',`<div class="settings-shell">
    <div class="settings-intro"><div><span>RAFLO FINANCE</span><h2>Configurações</h2><p>Centralize aqui os parâmetros que controlam o aplicativo e os dados financeiros.</p></div><div class="settings-badge">● Sistema local</div></div>
  ...
  $('#cfg').onsubmit=e=>{e.preventDefault();db.balance=num($('#bal').value);db.settings.project=$('#project').value.trim()||'RAFLO FINANCE';db.categories.receita=$('#catsr').value.split(',').map(x=>x.trim()).filter(Boolean);db.categories.despesa=$('#catsd').value.split(',').map(x=>x.trim()).filter(Boolean);save();toast('Configurações salvas com sucesso');render('config')};
  $('#reset-data').onclick=()=>{if(confirm('Excluir todos os dados locais do RAFLO FINANCE?')){db=defaultDB();save();render('dashboard');toast('Dados locais excluídos')}}
}
/* RAFLO FINANCE V1.5 — Dívidas/Despesas unificadas, empréstimos como dívida, calendário semanal e dashboard executivo */
...
function debtForm(id=null){const d=id?db.debts.find(x=>x.id===id):null;layout(d?'Editar Dívida':'Nova Dívida',`<div class="card"><form id="debt-form" class="form"><div class="field"><label>Credor</label><input id="creditor" value="${esc(d?.creditor||'')}" required></div><div class="field"><label>Tipo</label><select id="kind"><option>Instituição</option><option>Pessoa física</option><option>Financiamento</option><option>Outros</option></select></div><div class="field"><label>Valor da dívida (R$)</label><input id="dvalue" type="number" step=".01" min="0" value="${d?.value??''}" required></div><div class="field"><label>Parcelas</label><input id="installments" type="number" min="1" value="${d?.installments||1}" required></div><div class="field"><label>Valor da parcela (R$)</label><input id="dpart" type="number" step=".01" min="0" value="${d?.installmentValue??''}" required></div><div class="field"><label>Total a pagar</label><input id="dtotal" type="number" step=".01" value="${d?.totalPayable??''}" readonly></div><div class="field"><label>Fonte do pagamento</label><input id="dpayfrom" value="${esc(d?.paymentSource||'')}" placeholder="Conta, caixa, carteira, banco..."></div><div class="field"><label>Primeiro vencimento</label><input id="firstdate" type="date" value="${d?.firstDate||today()}" required></div><div class="field"><label>Tipo de pagamento</label><select id="dpaymenttype"><option value="unico">Único</option><option value="parcelado">Parcelado</option><option value="recorrente">Recorrente — mesmo valor e mesma data</option></select></div><div class="field"><label>Status</label><select id="dstatus"><option value="previsto">Prevista</option><option value="pendente">Pendente</option><option value="pago">Paga / quitada</option></select></div><div class="field full"><label>Observação</label><textarea id="dnote">${esc(d?.note||'')}</textarea></div><div class="actions full"><button class="btn primary">Salvar dívida</button><button type="button" class="btn secondary" data-view="movimentos">Cancelar</button></div></form></div>`); if(d){$('#kind').value=d.kind;$('#dstatus').value=d.status||'previsto';if($('#dpaymenttype'))$('#dpaymenttype').value=d.paymentType||'parcelado'}const recalc=()=>$('#dtotal').value=loanTotalPayable($('#dvalue').value,$('#installments').value,$('#dpart').value).toFixed(2);['#dvalue','#installments','#dpart'].forEach(s=>$(s).addEventListener('input',recalc));recalc();$('#debt-form').onsubmit=e=>{e.preventDefault();const id=d?.id||uid(),value=num($('#dvalue').value),parts=Math.max(1,num($('#installments').value)),part=num($('#dpart').value),total=loanTotalPayable(value,parts,part),status=$('#dstatus').value,paymentType=$('#dpaymenttype')?.value||'parcelado',data={id,creditor:$('#creditor').value,kind:$('#kind').value,value,installments:parts,installmentValue:part,totalPayable:total,firstDate:$('#firstdate').value,paymentSource:$('#dpayfrom').value,status,paymentType,note:$('#dnote').value,remaining:total,createdAt:d?.createdAt||today()};if(d){Object.assign(d,data);db.transactions=db.transactions.filter(x=>x.debtId!==id)}else db.debts.push(data);for(let i=0;i<parts;i++){const dt=new Date(iso(data.firstDate));dt.setMonth(dt.getMonth()+i);const date=ymd(dt);db.transactions.push({id:uid(),date,value:part,category:data.kind,person:data.creditor,paymentSource:data.paymentSource,status:i===0&&status==='pago'?'pago':'previsto',weekAssigned:weekOf(date),note:`Dívida ${id} — parcela ${i+1}/${parts}`,type:'despesa',debtId:id,installment:i+1,paymentType:data.paymentType})}if(data.paymentType==='recorrente'&&!d)createRecurringRule({type:'despesa',value:part,date:data.firstDate,category:data.kind,person:data.creditor,paymentSource:data.paymentSource,description:`${data.creditor} — ${data.kind}`});save();toast('Dívida salva');debtsExpenses()}
}
...
function calendar(m=calendarCursor){
  ...
  <div class="card calendar-rule"><b>Como funciona a diária:</b> o RAFLO calcula o valor que ainda precisa ser coberto pelos compromissos em aberto, desconta o caixa disponível e as receitas previstas, e divide pelo número de dias disponíveis. Ao passar um dia, registrar uma receita, pagar uma conta ou marcar uma folga, o valor é recalculado automaticamente.</div>
...
function togglePaid(id){const x=db.transactions.find(z=>z.id===id);if(!x)return;if(x.status==='pago'){const info=overdueInfo(x.date);x.status=info.days>0?'atrasado':'previsto';x.priority=info.days>0?info.priority:''}else{x.status='pago';delete x.priority}save();toast(x.status==='pago'?'Pagamento/recebimento confirmado e refletido no caixa':'Pagamento revertido; status recalculado');render(view)}
function bindGlobal(){
...
function purchaseForm(cardId){
  ...
  layout('Nova compra no cartão de crédito',`<div class="card"><div class="card-context"><b>${esc(c.name)}</b><span>${esc(c.bank)} • limite ${money(c.limit)}</span></div><form id="purchase-form" class="form"><div class="field"><label>Data da compra</label><input id="pdate" type="date" value="${today()}" required></div><div class="field"><label>Valor total (R$)</label><input id="pvalue" type="number" step="0.01" min="0" required></div><div class="field"><label>Parcelas</label><input id="pparts" type="number" min="1" value="1"></div><div class="field"><label>Categoria</label><select id="pcat">${categories('despesa')}</select></div><div class="field"><label>Estabelecimento / origem</label><input id="pstore"></div><div class="field"><label>Fonte do pagamento / fatura será paga por</label><input id="ppayfrom" placeholder="Conta, caixa, carteira..."></div><div class="field"><label>Tipo de pagamento</label><select id="ppaymenttype"><option value="parcelado">Parcelado</option><option value="unico">Único</option><option value="recorrente">Recorrente — mesmo valor e mesma data</option></select></div><div class="field full"><label>Observação</label><textarea id="pnote" placeholder="Ex.: compra, empréstimo, finalidade ou referência..."></textarea></div><div class="actions full"><button class="btn primary">Lançar no cartão</button><button type="button" class="btn secondary" data-view="movimentos">Cancelar</button></div></form></div>`);
  $('#purchase-form').onsubmit=e=>{e.preventDefault();const total=num($('#pvalue').value),parts=Math.max(1,num($('#pparts').value)),base=total/parts,id=uid(),person=$('#pstore').value||c.name,paymentSource=$('#ppayfrom').value||c.name,paymentType=$('#ppaymenttype')?.value||'parcelado';
    const debt={id,cardId:c.id,creditor:c.name,kind:'Cartão de crédito',value:total,totalPayable:total,installments:parts,installmentValue:base,firstDate:$('#pdate').value,status:'ativo',remaining:total,note:$('#pnote').value,paymentSource,createdAt:today()};db.debts.push(debt);
    for(let i=0;i<parts;i++){const dt=new Date(iso($('#pdate').value));dt.setMonth(dt.getMonth()+i);const date=ymd(dt);db.transactions.push({id:uid(),date,value:base,category:$('#pcat').value,person,paymentSource,status:'previsto',weekAssigned:weekOf(date),note:`Cartão ${c.name} — parcela ${i+1}/${parts}. ${$('#pnote').value}`,type:'despesa',cardId:c.id,debtId:id,installment:i+1,paymentType});}if(paymentType==='recorrente')createRecurringRule({type:'despesa',value:base,date:$('#pdate').value,category:$('#pcat').value,person,paymentSource,description:`Cartão ${c.name} — ${$('#pnote').value||'recorrente'}`});
...
async function initApp(){
  try{
    ...
    console.error('RAFLO FINANCE init error:',err);
...
/* ============================================================
   RAFLO FINANCE V1.19 — UX FINALIZATION / ORGANIZAÇÃO FINANCEIRA
